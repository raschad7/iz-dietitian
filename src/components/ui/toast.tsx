...
function ToastClose({
  ...
  return (
    ...
    >
      {/*
        `Icon` as a child, never through `render` — it fills itself with
        `dangerouslySetInnerHTML` and Base UI's `render` clones the element and
        passes children into it, which React refuses.
      */}
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  if (type === "success") {
    icon = (
      <Icon name="check" className="text-status-complete-mark" />
    )
  }
  ...
  if (type === "warning") {
    icon = (
      <Icon name="attention" className="text-status-attention-fg" />
    )
  }

  if (type === "error") {
    icon = (
      <Icon name="attention" className="text-destructive" />
    )
  }
