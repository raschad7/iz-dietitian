 ...
 *
 * `CalendarViewGuard` replaces the week view with the day view on any screen
 * narrower than 48rem. While this predicate was `pathname === href`, the two
 * disagreed forever: the guide pushed `week`, the guard replaced it with `day`,
 * and the guide pushed `week` again — several times a second, for as long as
 * step 9 or 10 was on screen, rebuilding the element the spotlight was measuring
 * on every pass.
 *
 * ⚠ The second argument is always a **pathname**, and a pathname carries no
 * query string. The three calendar views share the one route `/app/calendar` and
 * are told apart by `?view=`, so every view a reader can be on arrives here as
 * the bare `/app/calendar` — which is precisely why the step's own href, which
 * does carry the query, can never match it by equality and has to match by root.
 */
describe('stepScreenMatches', () => {
  ...
  test('the calendar accepts the view a phone is redirected to', () => {
    /*
      The loop, in one assertion. Whichever view the guard settles on, the
      reader's pathname is the shared `/app/calendar` — so the step asking for
      the week is already satisfied and never pushes again.
    */
    expect(stepScreenMatches('/app/calendar?view=week', '/app/calendar')).toBe(true);
    /* The legacy per-view paths still redirect here, and still count while they
       are briefly the pathname. */
    expect(stepScreenMatches('/app/calendar?view=week', '/app/calendar/day')).toBe(true);
    expect(stepScreenMatches('/app/calendar?view=week', '/app/calendar/month')).toBe(true);
  });
  ...
  test('the calendar is still somewhere the tour will navigate to', () => {
    ...
    */
    expect(stepScreenMatches('/app/calendar?view=week', '/app/clients')).toBe(false);
    expect(stepScreenMatches('/app/calendar?view=week', '/app')).toBe(false);
  });

  test('a sibling route that merely starts the same way is not a calendar view', () => {
    expect(stepScreenMatches('/app/calendar?view=week', '/app/calendar-settings')).toBe(false);
  });
  ...
  test('the calendar steps are walked to the day view on a phone', () => {
    ...
    */
    expect(stepHrefForScreen('/app/calendar?view=week', true)).toBe('/app/calendar?view=day');
  });

  test('a wide screen is left on the view the step asked for', () => {
    expect(stepHrefForScreen('/app/calendar?view=week', false)).toBe('/app/calendar?view=week');
  });

  test('every other step goes to its own screen at both widths', () => {
    for (const step of GUIDE_STEPS) {
      if (step.href === '/app/calendar?view=week') continue;
