 ...
 * until someone asks for the rest, and they carry `.q-field-primary` so the
 * difference is visible rather than merely positional.
 */
    ...
    preferredLocale: (
      ...
      >
        <Select
          ...
          defaultValue={client?.preferredLocale ?? locale}
        >
          {locales.map((value) => (
  ...
  return (
    <form action={formAction} className="flex min-h-0 flex-1 flex-col text-start">
      ...
      <DialogBody className="min-h-0 flex-1 gap-0 overflow-y-auto p-4 sm:p-5">
        {/*
          One column while the card is narrow; the same two-column grid as the
          hidden half once it has widened, so the two sets stay aligned.
        */}
        ...
        {/*
          ...
          The clipper's padding is cancelled by its own negative margin: it
          exists only so a focus halo, which paints 3px outside the field, has
          somewhere to land instead of being cropped by `overflow-hidden`.
        */}
        <div
          id={detailsId}
          className={cn(
        ...
        >
          <div inert={!expanded} className="-mx-1 overflow-hidden px-1 pb-1">
